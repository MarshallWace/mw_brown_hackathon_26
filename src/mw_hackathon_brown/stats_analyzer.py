...
def analyze_player_stats(mention: PlayerMention, stats: dict[str, Any]) -> StatsAnalysis:
    ...
    relevant_stat_names = POSITION_STATS.get(mention.position, [])
    if not relevant_stat_names:
        return analysis

    # Extract and score relevant stats
    for stat_name in relevant_stat_names:
        if stat_name in stats:
            raw_value = stats[stat_name]
            parsed = _parse_stat_value(raw_value)
            if parsed is not None:
                analysis.relevant_stats[stat_name] = raw_value
                score = _stat_to_score(stat_name, parsed)
                if score is not None:
                    analysis.stat_scores[stat_name] = score

    # Compute average score from scored stats
    if analysis.stat_scores:
        avg_score = sum(analysis.stat_scores.values()) / len(analysis.stat_scores)
        analysis.computed_score = round(avg_score)
        analysis.delta = mention.sentiment_score - analysis.computed_score

        # Determine assessment
        if abs(analysis.delta) <= 1:
            analysis.assessment = "justified"
        elif analysis.delta > 1:
            analysis.assessment = "overrated"
        else:
            analysis.assessment = "underrated"
