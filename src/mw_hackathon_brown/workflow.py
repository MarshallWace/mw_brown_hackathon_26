...
class NFLSentimentWorkflow(Workflow):
    ...
    async def match_players(self, ctx: Context, ev: SentimentExtractionEvent) -> PlayerMatchingEvent:
        ...
        matcher: PlayerMatcher = await ctx.store.get("matcher")

        print_separator("=")
        print("SENTIMENT VS STATS ANALYSIS")
        print_separator("=")

        unique_mentions, matches = await matcher.match_all_async(ev.results)

        return PlayerMatchingEvent(unique_mentions=unique_mentions, matches=matches)
